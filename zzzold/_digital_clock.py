...
class DigitalClock(CanvasWidget):
  ...
  def getStyleData(cls, ) -> dict[str, Any]:
    ...
    out = {}
    clsNames = ['SevenSegmentDigit', 'ColonDisplay', 'DigitalClock', ]
    for name in clsNames:
      styleId = 'statusBarClock'
      data = {
        ...
        'lowPen'         : parsePen(QColor(0, 0, 0, 0), 1, SolidLine),
        'highPen'        : parsePen(QColor(0, 0, 0, 0), 1, SolidLine),
        'lowBrush'       : parseBrush(QColor(144, 255, 0, 255), SolidFill),
        'highBrush'      : parseBrush(QColor(127, 0, 0, 255), SolidFill),
        'spacing'        : 1,
        'aspect'         : 0.2,
      }
      for key, value in data.items():
        styleKey = '%s/%s/%s' % (name, styleId, key)
        out[styleKey] = value
    return out
