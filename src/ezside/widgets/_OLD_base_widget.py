...
from ezside.core import alignDict, AlignFlag, Center, parseParent
...
class BaseWidget(_ParserWidget):
  ...
  def __init__(self, *args, **kwargs) -> None:
    ...
    if parent is None:
      _ParserWidget.__init__(self)
    elif isinstance(parent, QWidget):
      _ParserWidget.__init__(self, parent)
    else:
      e = typeMsg('parent', parent, QWidget)
      raise TypeError(e)

  def getMain(self, ) -> BaseWindow:
    """Getter-function for the main window"""
    main = self
    while isinstance(main.getOwningInstance(), BaseWidget):
      main = main.getOwningInstance()
      if getattr(main, '__main_window__', None) is not None:
        if TYPE_CHECKING:
          assert isinstance(main, BaseWindow)
        return main
